...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cond2plot = [1 2 3 4];          % need to update!!
condition = '2AFC';
figure();
plotKinTracking_CtrlvsStim(params,obj,sessix,cols,kin,offset,smooth,kinfeat,featix,trix2plot,cond2plot,stim,condition);

cond2plot = [5 6 7 8];          % need to update!!
condition = 'AW';
figure();
plotKinTracking_CtrlvsStim(params,obj,sessix,cols,kin,offset,smooth,kinfeat,featix,trix2plot,cond2plot,stim,condition);
%% Calculate percentage of time where tongue is visible during stim period
cond2plot = 2:13;           % with reference to params.condition

stim.stimstart = 0;         % Beginning of time window that you want to calculate tongue visibility for
stim.stimstop = 0.6;        % End of time window that you want to calculate tongue vis. for 

featix = find(strcmp(kin(sessix).featLeg,'tongue_length'));         % Find the index within the kinematic features that corresponds to 'tongue_length'

for sessix = 1:length(meta)             % For every session...
    pctTime = NaN(1,length(cond2plot)); % [1 x num conditions]

    for cond = 1:length(cond2plot)      % For every condition...
        stimstart = stim.stimstart;                             % Start time of window that you want
        startix = find(obj(sessix).time>stimstart,1,'first');   % Get the corresponding index in the time vector
        stimstop = stim.stimstop;                               % Stop time of window that you want
        stopix = find(obj(sessix).time<stimstop,1,'last');      % Get the corresponding index in the time vector

        condtrix = params(sessix).trialid{cond2plot(cond)};     % Trials that correspond to this condition
        tempTime = NaN(1,length(condtrix));                     % [1 x num trials in this condition]
        for t = 1:length(condtrix)                              % For all trials in this condition...
            currtrial = condtrix(t);                                            % Get the current trial                          
            tonguedat = kin(sessix).dat_std(startix:stopix,currtrial,featix);   % Get the tongue length for the current trial for the time window of interest
            dur = length(tonguedat);                                            % Number of time windows within the time window
            tonguevis = length(find(~isnan(tonguedat)));                        % Find the number of time bins that the tongue is visible (not a NaN value)
            tempTime(t) = tonguevis/dur;                                        % Fraction of time that tongue is visible = time bins where tongue is visible / total time bins in window 
        ...
        pctTime(cond) = mean(tempTime,'omitnan');               % Pct time for each condition = average across all trials the fraction of time that the tongue was visible
...
pctTime(sess2omit,:) = [];
pctTime = 100*pctTime;            % [sessions x conditions]
...
perf_byanm = NaN(length(uniqueAnmNames),size(pctTime,2));  % [# animals, # conditions]
for anms = 1:length(uniqueAnmNames)                         % For each animal...
    ...
    perf_byanm(anms,:) = mean(pctTime(anmix,:), 1);            % Get the mean 'performance' for the current animal
end
%% Plot
...
[AFCpval, AWpval] = plotPerformance_Bar_Scatter(anmNames, cols, pctTime, perf_byanm, ...
...
for test = 1:(length(conds2plot)/2)             % Three t-tests (comparing each of the pairs of conditions)
    ...
    disp(num2str(hyp))
    if hyp&&test==1
        scatter(1.5,55,30,'*','MarkerEdgeColor','black')
    elseif hyp&&test==2
        scatter(3.5,55,30,'*','MarkerEdgeColor','black')
    elseif hyp&&test==3
        scatter(5.5,55,30,'*','MarkerEdgeColor','black')
    end
end
xlim([0 7])
xticks([1,2,3,4,5,6])
...
ylim([0 60])
set(gca, 'TickDir', 'out')
ylabel('Pct of time with tongue visible')
