...
params.condition(end+1) = {'autowater&~early&~no'};                         % R 2AFC hits, no stim
params.condition(end+1) = {'~early&~no'};
...
params.traj_features = {{'tongue','left_tongue','right_tongue','jaw','trident','nose'},...
    {'top_tongue','topleft_tongue','bottom_tongue','bottomleft_tongue','jaw','top_nostril','bottom_nostril','top_paw'}};
...
%% Plot all trials for the same feature in one subplot
cond2use = 4;

feat2use = {'jaw_yvel_view1','nose_yvel_view1','top_paw_yvel_view2','motion_energy'};
...
for sessix = 1:length(meta)
    ...
    for c = 1:length(cond2use)
        ...
        for feat = 1:length(featix)
            subplot(2,2,feat)
            condkin = kin(sessix).dat_std(:,trix,featix(feat));
            for tt = 1:nTrixPlot
                toplot = offset*tt+abs(mySmooth(condkin(:,tt,:),sm));
