...
%     kinfeats{i} = cat(3,kinfeats{i},reshape(len,size(len,1),size(len,2),1));
    ...
    kin(i).dat_reduced = reduceDimensionVideoFeatures(kinfeats{i},dfparams.feat_varToExplain,size(gpfa(i).gpfalatents,2));
...
for i = 1:numel(meta)
    gpfadat = permute(gpfa(i).gpfalatents,[1 3 2]);
    kindat = kin(i).dat_reduced;
    save(fullfile('input_data',[meta(i).anm '_' meta(i).date]),'gpfadat','kindat');
end
