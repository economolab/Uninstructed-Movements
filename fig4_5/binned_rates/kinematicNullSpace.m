...
% data_structure_JEB6_2021-04-18.mat
% this session doesn't have new dlc, skipping it for now
% it's the first session, so keeping 2:end for everything
params.probe = params.probe(2:end);
params.trialid = params.trialid(2:end);
params.cluid = params.cluid(2:end);
meta = meta(2:end);
objs = objs(2:end);
me = me(2:end);
...
dfparams.traj_features = {{'tongue','jaw','nose'},...
                        {'top_tongue','topleft_tongue','bottom_tongue','bottomleft_tongue','jaw','top_paw','bottom_paw','top_nostril','bottom_nostril'}};
dfparams.advance_movement = 0.025;
dfparams.feat_varToExplain = 70;
for i = 1:numel(meta)
...
for i = 1:numel(meta)
    clear newrez
    input_data.factors = objs{i}.trialdat;
    input_data.feats = kinfeats_reduced{i};
    newrez = estimateW(objs{i},input_data,params,objs{i}.time,me(i)); % N,V are zscored neural activity and feature matrix
