% Predicting context from ordered null and potent dimensions (dimensions
% ordered in terms of variance explained)
% -------------------------------------------------------------------------------------
...
%% Order null and potent dimensions according to variance explained (first dimension explains the most variance)
for sessix = 1:numel(meta)
    for i = 1:2
        if i==1
            space = 'null';
        ...
        VE = rez(sessix).ve.(space);                        % Get the variance explained for each dimension within the null or potent space
        [~,VEix] = sort(VE,'descend');                      % Sort these VE values in descending order and get the order of the sorted indices
        spaceproj = rez(sessix).(spacename);                % Get the single trial projections onto all dimensions
        rez(sessix).(spacename) = spaceproj(:,:,VEix);      % Sort them in the same order as the sorted VE values
    end
end
