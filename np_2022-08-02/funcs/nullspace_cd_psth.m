function [rez,times] = nullspace_cd_psth(rez,obj,params)
...
rez.null_psth = rez.proj_null;
rez.potent_psth = rez.proj_potent;
...
nullmodes = zeros(size(rez.proj_null,2),numel(fns));
potentmodes = zeros(size(rez.proj_potent,2),numel(fns));
for i = 1:numel(fns)
...
for i = 1:numel(fns)
    nullmode = rez.cd.null.(fns{i});
    potentmode = rez.cd.potent.(fns{i});
    
    temp = rez.proj_null;
    for j = 1:size(temp,3) % condition
        rez.cd.null.([fns{i}(1:end-5) '_latent'])(:,j) = temp(:,:,j) * nullmode;
    end

    temp = rez.proj_potent;
    for j = 1:size(temp,3) % condition
        rez.cd.potent.([fns{i}(1:end-5) '_latent'])(:,j) = temp(:,:,j) * potentmode;
    end
