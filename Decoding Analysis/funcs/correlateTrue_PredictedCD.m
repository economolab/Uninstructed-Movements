function R2 = correlateTrue_PredictedCD(trueVals, modelpred)
...
for t = 1:size(trueVals.Rhit{sessix},2)                 % For every right hit trial...
    trueR = trueVals.Rhit{sessix}(1:end-1,t);   % Take the true CDlate value
    predR = modelpred.Rhit{sessix}(:,t);        % Take the predicted CDlate value
...
for t = 1:size(trueVals.Lhit{sessix},2)         % Do the same for left hit trials
    trueL = trueVals.Lhit{sessix}(1:end-1,t);
    predL = modelpred.Lhit{sessix}(:,t);
