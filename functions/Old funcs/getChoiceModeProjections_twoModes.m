...
for j = 1:numel(conditions)             % For each condition...
    ...
    if ~isempty(allModes.choice_mode)
        allModes.latentChoice{j} = mySmooth(psth(:,:,cond)*allModes.choice_mode,smooth);          % Project the trials from curr condition onto normal mode
        allModes_NoMove.latentChoice{j} = mySmooth(psth(:,:,cond)*allModes_NoMove.choice_mode,smooth);  % Project the trials from curr condition onto mode w/o early move
    end
end
