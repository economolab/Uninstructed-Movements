...
% 'auw' for autowater, animal, date)

% OUTPUT: saves the average probability of jaw movement at all timepoints
% in a trial (for one session)


function jawdat = jawkinSessionAvg(dir,context, animal, date)
    pth = ['C:\Users\Jackie\Documents\Grad School\Economo Lab\DataObjects\',animal,'\data_structure_',animal,'_',date,'.mat'];
    if ~ischar(pth) 
        pth = append(pth(1),pth(2),pth(3),pth(4),pth(5),pth(6),pth(7),pth(8),pth(9));
        pth = string(pth);
    end
    dat = load(pth);
    ...
    jawdat = nanmean(jaw,2);
    jawdat = medfilt1(jawdat,10);
    % thisses = nanmean(jaw,2);
    % s = plot(thisses,'color', clr, 'LineWidth', 0.5,'DisplayName',str); 
y = nanmean(jawdat,2);
%plot(edges, y,'color', clr, 'LineWidth', 2);
end % jawkinSessionAvg
%%
