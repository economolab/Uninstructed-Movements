...
for sessix = 1:numel(objs)
    ...
    rez(sessix).cdEarly_mode = Early; 
    rez(sessix).cdLate_mode = Late;  rez(sessix).cdGo_mode = Go;
...
for gg = 1:numel(objs)
    ...
    for i = 1:numel(kinfns)
        Y = modekin.(kinfns{i}); % feature data to use to calculate mode
        ...
        % Only find kinematic modes based on the specified condition
        [kinmode.(kinfns{i}), dat.(kinfns{i})] = findMode(obj, Y, params,modecond);
        modename = strcat('cd',kinfns{i},'_mode');
        rezNoJaw(gg).(modename) = kinmode.(kinfns{i});
    end
    

    %%% Re-arrange kin fieldnames according to the order in which you want to
    %%% orthogonalize them
    fns = reorderFns(rezNoJaw,gg);
...
function fns = reorderFns(rezNoJaw,gg)
