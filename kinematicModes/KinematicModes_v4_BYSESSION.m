...
%     %params.tix = 1:1000;
    conditions = 1:2;
    [mode,dat,proj,kinfns] = calcKinModes(kin,obj,params,psthForProj,conditions);
    ...
    % VARIANCE EXPLAINED %
    varexp_bothcond(gg) = findVarExplained(obj,orthmode,kinfns);
...
%%
VEone.ME = [];  VEboth.ME = [];
VEone.jaw = [];  VEboth.jaw = [];
VEone.nose = [];  VEboth.nose = [];
for gg = 1:length(meta)
    VEone.ME = [VEone.ME, varexp_onecond(gg).MEinterp];  VEboth.ME = [VEboth.ME, varexp_bothcond(gg).MEinterp];
    VEone.jaw = [VEone.jaw, varexp_onecond(gg).jawVel];  VEboth.jaw = [VEboth.jaw, varexp_bothcond(gg).jawVel];
    VEone.nose = [VEone.nose, varexp_onecond(gg).noseVel];  VEboth.nose = [VEboth.nose, varexp_bothcond(gg).noseVel];
end
%%
figure();
x = [1,2,4,5,7,8];
y = [mean(VEone.ME),mean(VEboth.ME,'omitnan'),mean(VEone.jaw),mean(VEboth.jaw,'omitnan'),mean(VEone.nose),mean(VEboth.nose,'omitnan')];
b= bar(x,y);
b.FaceColor = [0.75 0.75 0.75]; hold on;


scatter(1,VEone.ME,'black','filled'); scatter(2,VEboth.ME,'magenta','filled')
scatter(4,VEone.jaw,'black','filled');  scatter(5,VEboth.jaw,'magenta','filled')
scatter(7,VEone.nose,'black','filled');  scatter(8,VEboth.nose,'magenta','filled')
yy = [VEone.ME',VEboth.ME']; plot(x(1:2),yy(:,1:2),'Color','black')
yy = [VEone.jaw',VEboth.jaw']; plot(x(3:4),yy(:,1:2),'Color','black')
yy = [VEone.nose',VEboth.nose']; plot(x(5:6),yy(:,1:2),'Color','black')

xlim([0 9])
ylabel('Variance explained (out of 1)')
title('Difference in VE using R trials only vs both trial types')
%% FUNCTIONS
