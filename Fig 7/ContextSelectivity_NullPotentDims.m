% Find context selectivity across all Null and potent dimensions
% Take the difference between contexts for each dimension.  Take the mean
% across all dimensions within a session
% -------------------------------------------------------------------------------------
...
for sessix = 1:numel(meta)
    for i=1:2
        ...
        tempSel = proj(:,:,conds2use(1))-proj(:,:,conds2use(2));
        for d = 1:nDims
            temp = tempSel(:,d);
            preSampAvg = mean(temp(start:stop,:),1);
            if preSampAvg<0
                temp = -1*(temp);
            end
            tempSel(:,d) = mySmooth(temp,21);
        end
