...
for gg = 1:length(meta)
    ...
    thirdR = round(numR/3); thirdL = round(numL/3);
    averageR.begin(gg) = mean(moveDelay.right{gg}(1:thirdR),2,'omitnan');  averageL.begin(gg) = mean(moveDelay.left{gg}(1:thirdL),2,'omitnan');
    averageR.middle(gg) = mean(moveDelay.right{gg}((thirdR+1):(2*thirdR)),2,'omitnan');  averageL.middle(gg) = mean(moveDelay.left{gg}((thirdL+1):(2*thirdL)),2,'omitnan');
    averageR.late(gg) = mean(moveDelay.right{gg}((end-thirdR):end),2,'omitnan');  averageL.late(gg) = mean(moveDelay.left{gg}((end-thirdL):end),2,'omitnan');
end
%%
