function reformattedKin = formatKin(kinData,params,time,cond2use)
...
reformattedKin = NaN(length(time),size(kinData,3),length(cond2use));     % Want kinematic data to be in format (time x nMoveFeat x conditions)
...
for c = 1:length(cond2use)                                                 % For each condition...
    ...
    condkin = kinData(:,condtrix,:);                     % Grab the kinematic data for desired features from those trials
