...
% % soft-normalize (like in elsayed paper)     % DOESNT SEEM VERY IMPT
% % normalization factor=firing rate range+5
% for i = 1:size(N,3) % for each neuron/factor
%     normfactor(i) = max(max(N(:,:,i))) - min(min(N(:,:,i)));
%     N(:,:,i) = N(:,:,i) ./ normfactor(i);
% end
% d = 4;
% figure;
% subplot(1,2,1)
