...
aw = '1';
params.modecondition(6) = {['R&hit&autowater.nums==' aw '&stim.num==' stim '&~early']};  %rhit, aw on 
params.modecondition(7) = {['L&hit&autowater.nums==' aw '&stim.num==' stim '&~early']};  %lhit, aw on 
...
epoch = 'delay'; 
rez.choice_mode = choiceModeLDA(obj,params,cond,epoch,rez.alignEvent);
clear cond
...
cond{1} = params.modecondition{1};
cond{2} = params.modecondition{2};
cond{3} = params.modecondition{6};
cond{4} = params.modecondition{7};
epoch = 'action'; 
rez.action_mode = actionModeLDA(obj,params,cond,epoch,rez.alignEvent);
clear cond

%% all time points mode
cond{1} = params.modecondition{1};
cond{2} = params.modecondition{2};
cond{3} = params.modecondition{6};
cond{4} = params.modecondition{7};
epoch = 'action'; 
rez.all_mode = allTimeModeLDA(obj,cond);
clear cond
...
plt.title = '2AFC-AW (Correct) Trials [Jaw Onset]';
