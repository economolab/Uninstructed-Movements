...
for i = 1:size(gauss,2) % for each clu
    gauss(:,i,:) = mySmooth(squeeze(gauss(:,i,:)),sigma); % causal gaussian filter
end
...
for j = 1:2
    for i = 1:size(gauss{j},2) % for each clu
        gauss{j}(:,i,:) = mySmooth(squeeze(gauss{j}(:,i,:)),N,sigma); % causal gaussian filter
    end
end
...
for j = size(gauss)
    for i = 1:size(gauss{j},2) % for each clu
        [gauss{j}(:,i,:),kernsd] = mySmooth(squeeze(gauss{j}(:,i,:)),N); % causal gaussian filter
    end
end
...
trix{1} = randsample(1:size(gauss{1},3),25);
trix{2} = randsample(1:size(gauss{2},3),25);
...
for i = 1:numel(clus)
    ...
    for j = 1:numel(gauss)
        temp = squeeze(gauss{j}(:,ix,trix{j}));
    ...
    for j = 1:numel(lfads)
        temp = squeeze(lfads{j}(:,ix,trix{j}));
    ...
    title(['Gaussian | sigma=' num2str(kernsd*params.dt*1000) ' ms'])
    xlim([obj.time(10) obj.time(end)])
    xlabel('Time (s) from go cue')
    ax.YTick = [];
    ax = gca;
