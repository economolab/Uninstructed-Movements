...
y = cat(1,ones(numel(trials.rlfads),1),2*ones(numel(trials.llfads),1));


ce_lfads = zeros(size(X,1),1);
acc_lfads = ce_lfads;
for i = 1:size(X,1) % each timepoint
    ...
    acc_lfads(i) = sum(Mdl_lfads.kfoldPredict == y) / numel(y);
end
...
for i = 1:size(X,1) % each timepoint
    ...
    acc_pca(i) = sum(Mdl_pca.kfoldPredict == y) / numel(y);
end

% smoothing

X = obj.trialdat(:,:,dat.trials);
X = obj.trialdat(:,:,trials.nums);


ce_smooth = zeros(size(X,1),1);
acc_smooth = ce_smooth;
for i = 1:size(X,1) % each timepoint
    x = squeeze(X(i,:,:)); % (factors,trials) for timepoint i
    [Mdl_smooth] = fitclinear(x',y,'KFold',5,...
        'Learner','logistic');
    ce_smooth(i) = kfoldLoss(Mdl_smooth);
    acc_smooth(i) = sum(Mdl_smooth.kfoldPredict == y) / numel(y);
end

% lfads rates

X = dat.rates(:,:,trials.nums);


ce_rates = zeros(size(X,1),1);
acc_rates = ce_rates;
for i = 1:size(X,1) % each timepoint
    x = squeeze(X(i,:,:)); % (factors,trials) for timepoint i
    [Mdl_rates] = fitclinear(x',y,'KFold',5,...
        'Learner','logistic');
    ce_rates(i) = kfoldLoss(Mdl_rates);
    acc_rates(i) = sum(Mdl_rates.kfoldPredict == y) / numel(y);
end
...
plot(obj.time,ce_lfads,'Color',[70, 176, 106]./255,'LineWidth',3)
plot(obj.time,ce_pca,'Color',[115, 68, 130]./255,'LineWidth',3)
plot(obj.time,ce_smooth,'Color',[75, 110, 166]./255,'LineWidth',3)
plot(obj.time,ce_rates,'Color',[171, 72, 95]./255,'LineWidth',3)
...
ylabel('Error')
legend({'LFADS','PCA','Smoothing','Rates'},'Location','best')
ax = gca;
ax.FontSize = 20;
...
plot(obj.time,acc_smooth,'Color',[75, 110, 166]./255,'LineWidth',3)
plot(obj.time,acc_rates,'Color',[171, 72, 95]./255,'LineWidth',3)
...
xlabel('Time (s) from go cue')
ylabel('Accuracy')
legend({'LFADS factors','PCA latents','Smoothing','Rates'},'Location','best')
ax = gca;
ax.FontSize = 20;
xlim([obj.time(30) obj.time(end)])
hold off
