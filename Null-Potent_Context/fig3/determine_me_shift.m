% determine best time shift b/w motion energy and neural activity
...
for sessix = 1:numel(meta)
    ...
    % -- motion energy
    me_zscored = zscore(me(sessix).data);
...
r_mean_across_trials = cellfun(@(x) squeeze(mean(x,2)),r,'UniformOutput',false);
r_mean_across_neurons = cellfun(@(x) squeeze(mean(x,2)),r_mean_across_trials,'UniformOutput',false);
