% DECODING CDRamp FROM ALL KINEMATIC FEATURES (with ridge regression,
...
% trials
par.cond2use = [2 3]; % With reference to params.condition -- CHANGE this for each CD prediction %%%%%%%%%%%%%%%%
...
% Times that you want to use to baseline normalize CDRamping
...
xlim(ax2,[-2.3 0])
set(ax2,'TickDir','out'); 
%% Make heatmaps for a single session showing CDRamp across trials and predicted CDRamp
plotheatmap = 'yes';
sm = 15;
...
if strcmp(plotheatmap,'yes')
    ...
    cond2plot = {'Lhit','Rhit'};
    for sessix = 1:length(meta) %exsess                                                                  % For each session...
        figure();
        cnt = 0;
        tempTrue = [];
        tempPred = [];
...
trial =1;
for cell = 1:10
    plot(obj(1).time,obj(1).trialdat(:,cell,trial))
    pause
end
%%
feature = 'tongue_length';
featix = find(strcmp(kin(1).featLeg,feature));
offset = 10;
for trial = 1:100
    plot(obj(1).time,(trial*offset)+kin(1).dat(:,trial,featix));
    hold on
end
%% FUNCTIONS
