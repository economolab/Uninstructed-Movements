...
cd_ramping = getCodingDimensions_2afc(obj,params,cond2use,rampcond,cond2proj);
...
% gocond = 8;
% cond2proj = [2,3,6,7];
% cds = getCodingDimensions_2afc(obj,params,cond2use,gocond,cond2proj);
...
cd = 'ramping';
cd_ramping = getSingleTrialProjs(cd_ramping,obj,cd);
%%
colors = getColors();
cond2proj = [1 2 5 6];
mecond2proj = [2 3 6 7];
...
%         ax1 = subplot(1,2,1);
        toplot = cd_ramping(sessix).cd_proj(:,cond2proj(c),3);
        tempRamp(:,c) = toplot; 
...
%         hold(ax2, 'on');
    ...
    Ramping_all{sessix} = tempRamp;
    ME_all{sessix} = tempME;
...
%% 
Ramping_reorg = cell(1,length(cond2proj));
ME_reorg = cell(1,length(cond2proj));
for c = 1:length(cond2proj)
    tempR = [];  tempM = [];
    for sessix = 1:length(meta)
        tempR = [tempR, Ramping_all{sessix}(:,c)];
        tempM = [tempM, ME_all{sessix}(:,c)];
    end
    Ramping_reorg{c} = tempR;
    ME_reorg{c} = tempM;
end
%%
for c = 1:length(cond2proj)
        ...
        ax1 = subplot(1,2,1);
        toplot = mean(Ramping_reorg{c},2,'omitnan');
        plot(obj(sessix).time,mySmooth(toplot,31),'Color',col,'LineStyle',linestyl,'LineWidth',lw)
        hold(ax1,'on');

        ax2 = subplot(1,2,2);
        condME = mean(ME_reorg{c},2,'omitnan');
        plot(obj(sessix).time,mySmooth(condME,31),'Color',col,'LineStyle',linestyl,'LineWidth',lw)
        hold(ax2, 'on');
end
legend(ax1,'R hit', 'L hit','R ignore','L ignore')
xlim(ax1,[-2.4 2.5]); xlim(ax2,[-2.4 2.5])
title(ax1,'Ramping')
