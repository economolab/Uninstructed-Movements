% Calculating Ramping mode in all sessions and relating it to delay period
% motion energy
...
%% Store all of the R2 values for each session
JEB14_delR2 = NaN(length(meta),1);
for sessix = 1:length(meta)
    JEB14_delR2(sessix) = obj(sessix).R2;
end
%% Plot bar plot to show average R2 values across sessions
colors = getColors_Updated();
load('C:\Users\Jackie\Documents\Grad School\Economo Lab\Code\Uninstructed-Movements\Fig 2\Ramping mode\delR2Vals_NoJEB14.mat')
delR2_ALL = [delR2Vals_NoJEB14; JEB14_delR2];                              % Group the R2 values for JEB14 and all other animals
anmNames = {'JEB6','JEB7','JEB7','EKH1','EKH3','EKH3','JGR2','JGR2','JGR3',...
    'JEB13','JEB13','JEB13','JEB13','JEB13','JEB13','JEB15','JEB15','JEB15'};
JEB14_anmNames = {'JEB14','JEB14','JEB14','JEB14'};
anmNames_all = [anmNames,JEB14_anmNames];                               % Animal names for each session
nSessions = numel(anmNames_all);

uniqueAnm = unique(anmNames_all);

exsess = 1;                                                            % The index of the session that you want to be highlighted
markerSize = 60;
figure();
...
for sessix = 1:nSessions
    ...
    scatter(1,delR2_ALL(sessix),markerSize,'filled',shape,'MarkerFaceColor',colors.afc); hold on;
end
