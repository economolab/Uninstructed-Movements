...
clearvars -except Normselectivity Normselectivity_Ctrl zerodif selectivity_All selectivity_AllCtrl obj meta params kin ctrlobj ctrlmeta ctrlparams ctrlkin
...
figure();
toplot = mean(Normselectivity,2,'omitnan');
%err = 1.96*(std(selectivity_All,0,2,'omitnan')/sqrt(size(selectivity_All,2)));
err = std(Normselectivity,0,2,'omitnan')/sqrt(size(selectivity_All,2));
ax = gca;
...
toplot = mean(Normselectivity_Ctrl,2,'omitnan');
%err = 1.96*(std(selectivity_AllCtrl,0,2,'omitnan')/sqrt(size(selectivity_AllCtrl,2)));
err = std(Normselectivity_Ctrl,0,2,'omitnan')/sqrt(size(selectivity_AllCtrl,2));
ax = gca;
