...
for sessix = 1:length(randmeta)
       cdproj = mySmooth(del(sessix).condProj{delix}(timeix,:),sm,'reflect');
...
% t2 = 0;
% t2ix = find(randobj(1).time<t2,1,'last');
% timeix.rand = t1ix:t2ix;
% restrictedTimeAx.rand = randobj(1).time(timeix.rand);

for sessix = 1:length(randobj)
    cdproj = mySmooth(del(sessix).condProj{randdelix},sm,'reflect');
    temp = cdproj(:,1)-cdproj(:,2);
%     plot(restrictedTimeAx,temp); hold off
% % %     pause
    CDsel.rand(:,sessix) = temp;
end
%%
% Times aligned to go cue
% t1 = -2.5;
...
%         t2ix = find(fixobj(1).time<t2,1,'last');
%     end
%     timeix.fix = t1ix:t2ix;
%     restrictedTimeAx.fix = fixobj(1).time(timeix.fix);

%     cdproj = mySmooth(squeeze(regr(sessix).cd_proj(timeix.fix,cond2use,1)),sm,'reflect');
     cdproj = mySmooth(squeeze(regr(sessix).cd_proj(:,cond2use,1)),sm,'reflect');
    temp = cdproj(:,1)-cdproj(:,2);
    CDsel.fix(:,sessix) = temp;
end
%% Standardize CDselectivity as all other CDprojections are standardized for this figure
t1 = -2.5;
t2 = -2;
t1ix = find(fixobj(1).time>t1,1,'first');
t2ix = find(fixobj(1).time<t2,1,'last');

fixmu = mean(CDsel.fix(t1ix:t2ix,:),1,'omitnan');
fixsigma = std(CDsel.fix(t1ix:t2ix,:),[],1,'omitnan');
CDselnorm.fix = (CDsel.fix - fixmu) ./ fixsigma;
...
% fixmu = mean(CDsel.fix,1,'omitnan');
% fixsigma = std(CDsel.fix,[],1,'omitnan');
% CDselnorm.fix = (CDsel.fix - fixmu) ./ fixsigma;
% 
% randmu = mean(CDsel.rand,1,'omitnan');
% randsigma = std(CDsel.rand,[],1,'omitnan');
% CDselnorm.rand = (CDsel.rand - randmu) ./ randsigma;
%%
% Fixed delay slope (of CDchoice selectivity)
t1 = -2.5;
t2 = 0;
t1ix = find(fixobj(1).time>t1,1,'first');
t2ix = find(fixobj(1).time<t2,1,'last');
...
for sessix = 1:nSess
    tempslope = diff(CDselnorm.fix(t1ix:t2ix,sessix));
    slope.fix(:,sessix) = mySmooth(tempslope,smooth);
end

% Randomized delay slope (of CDchoice selectivity)
t1 = -2.5;
t2 = 0;
t1ix = find(randobj(1).time>t1,1,'first');
t2ix = find(randobj(1).time<t2,1,'last');
...
% err = 1.96*(mySmooth(std(CDselnorm.fix(t1ix:t2ix,:),0,2),sm)./sqrt(nSess));
...
% err = 1.96*(mySmooth(std(CDselnorm.rand,0,2),sm)./sqrt(nSess));
